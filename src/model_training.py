...
def save_training_artifacts(model, history, class_indices, base_path="models"):
    """Save all training artifacts to the models directory"""
    ...
    # 3. Save training history (optional)
...
def train_and_save_model(train_data, val_data, num_classes, input_shape=(224, 224, 3)):
    ...
    for layer in model.layers[1].layers[:-20]:  # Freeze all but last 20 layers
...
if __name__ == "__main__":
    # First import your data generators
    ...
    model, history, artifacts = train_and_save_model(
        ...
        val_data=val_data,
        num_classes=len(train_data.class_indices)  # Automatically get number of classes
    )
